...
#     val = fom.multifom_uniform(gratingTorcwa, choose_monofom, final_speed=20., goal=0.1, return_grad=True)
# total = time.time()-start

# print(f'TORCWA time (average over {len(ps)} runs) = {total/len(ps)}')

# Profile this function to see which parts are slowest
val = fom.multifom_uniform(gratingTorcwa, choose_monofom, final_speed=20., goal=20, return_grad=True)
print(val)