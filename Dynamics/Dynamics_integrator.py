...
if nonlinear_run:
    lookup_data_fname = f'./Data/{runID_load}_Lookup_table_lambda_{klambda}_by_delta_{kdelta}_allangles.pkl'
else:
...
# Custom
y0      = 0.*w  # metres
phi0    = 0.*np.pi/180  # degrees converted to radians
vy0     = 0.  # metres per second
omega0  = 1.*2*np.pi  # revolutions per second converted to radians per second

# time_MAX = 72.*60*60  # Maximum runtime (seconds)
time_MAX = 120  # Maximum runtime (seconds)
velocity_MAX = 0.2*c
h = 1e-3   # Step size  
runID = runID_load + f"_{omega0/(2*np.pi):.1f}rps"  # For saving dynamics data
